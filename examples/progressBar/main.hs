...
update model StartTimer = if model.running
  ...
  else (model { running = True, value_ = 0 }, tick)
update model Tick = if model.value_ > 100
  then (model { running = False }, Command.none)
  else (model { value_ = model.value_ + 0.5 }, tick)

view :: Model -> Element
view model =
  center [] $
  column [alignItems Center, spacing 50] [
    progressBar [width $ Fixed 300, height $ Fixed 30] 0 100 model.value_,
    button [onPressIf (not model.running) StartTimer] "Start"
  ]

main :: IO ()
main = Iced.run [theme Oxocarbon] "ProgressBar" model update view
  where model = Model { value_ = 0, running = False }
