...
shoot model = case model.shooting of
  True -> do
   ...
   where
    bullets = model.bullets ++ [makeBullet model.position]
  False -> pure model

makeBullet :: Position -> Bullet
makeBullet ship = Bullet{..}
 where
  position = Position{x = ship.x - 2, y = ship.y}
...
view model = container [centerX Fill, centerY Fill] $
  case model.screen of
    ...
    Win ->
      column
        [alignX Alignment.Center, spacing 30]
...
main = do
  model <- newModel
  Iced.run
    [subscription subscriptionFn, theme Oxocarbon]
    "Space shooter"
    model
    update
    view
