...
#[no_mangle]
extern "C" fn svg_handle_from_bytes(len: usize, array_ptr: *const u8) -> *mut Handle {
    let bytes = read_vec(len, array_ptr);
    Box::into_raw(Box::new(Handle::from_bytes(bytes)))
}

#[no_mangle]
extern "C" fn svg_handle_from_rgba(
    width: c_uint,
    height: c_uint,
    len: usize,
    array_ptr: *const u8,
...
#[no_mangle]
extern "C" fn svg_new(handle_ptr: *mut Handle) -> SelfPtr {
    let handle = unsafe { *Box::from_raw(handle_ptr) };
    Box::into_raw(Box::new(svg(handle)))
}

#[no_mangle]
extern "C" fn svg_width(self_ptr: SelfPtr, width: *mut Length) -> SelfPtr {
...
#[no_mangle]
extern "C" fn svg_height(self_ptr: SelfPtr, height: *mut Length) -> SelfPtr {
    let image = unsafe { Box::from_raw(self_ptr) };
...
#[no_mangle]
extern "C" fn svg_into_element(self_ptr: SelfPtr) -> ElementPtr {
    let image = unsafe { *Box::from_raw(self_ptr) };
